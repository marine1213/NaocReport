...
//Note: if the page keep reloading from signIn page, the chances are the Drive API for new project has not been enabled, goto: https://console.developers.google.com/apis/api/drive.googleapis.com/overview to enable the Drive API for proper project.
testPostJSON=()=>{
...
async function postMultipart(file = {},accessToken) {
  ...
  var metadata={
      ...
      'parents': ['18EIvVrJfd8Er_sjZPT1WgQ2VoiRpG4B0'], // Folder ID at Google Drive
  };
...
async function deleteFile(fileId, accessToken) {
  ...
  response.status==204?logE(response):logE(null,'File chưa được xóa trên CSDL!');
